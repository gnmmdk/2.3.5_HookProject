...
//          HookCallback也要处理
            customLoadedApkAction();//目前只适配了7.1.1
        } catch (Exception e) {
            e.printStackTrace();
                ...
                case LAUNCH_ACTIVITY:
                    try {
                        ...
                        if(actionIntent!=null){
                            ...
                            if(actionIntent.getPackage() == null){// todo 证明是插件
...
//        addAssetPathMethodPlugin.setAccessible(true);//public 不需要setAccessible
    ...
    private void customLoadedApkAction() throws Exception{
        ...
        //todo 3 自定义一个ClassLoader设置给LoadedApk的成员变量mClassLoader。
        ...
        //todo 1 增加了我们自己的LoadedApk到ActivityThread里面的mPackages成员变量
        mPackages.put(applicationInfo.packageName,weakReference);
    }

    /**
     * todo 3 获取到applicationInfo
     * @return
     * @throws Exception
     */
...
//        final Object mIPackageManagerObj = mGetPackageManagerMethod.invoke(null);
        ...
        Object mIPackageManagerProxy = Proxy.newProxyInstance(getClassLoader(), new Class[]{mIPackageManagerCLazz}, new InvocationHandler() {
            @Override
            public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                if(method.getName().equals("getPackageInfo")){// todo 4.2  替换
